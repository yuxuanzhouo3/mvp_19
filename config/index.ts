const PUBLIC_DEPLOYMENT_REGION = (process.env.NEXT_PUBLIC_DEPLOYMENT_REGION || process.env.NEXT_PUBLIC_SITE_REGION || '').trim().toUpperCase()
const SERVER_DEPLOYMENT_REGION = (process.env.DEPLOYMENT_REGION || process.env.SITE_REGION || '').trim().toUpperCase()
const PUBLIC_SITE_URL = (process.env.NEXT_PUBLIC_APP_URL || '').trim().toLowerCase()
...
export function getDeploymentRegion(): 'CN' | 'INTL' {
  ...
  if (typeof window !== 'undefined' && window.location?.hostname) {
    ...
    if (host.includes('mornscience.top') || host.includes('.cn')) {
      return 'CN'
    }
    if (host.includes('mornscience.work') || host.includes('.com') || host.includes('.org')) {
      return 'INTL'
    }
  }
  ...
  if (siteUrl) {
    if (siteUrl.includes('mornscience.top') || siteUrl.includes('.cn')) {
      return 'CN'
    }
    if (siteUrl.includes('mornscience.work') || siteUrl.includes('.com') || siteUrl.includes('.org')) {
      return 'INTL'
    }
  }
