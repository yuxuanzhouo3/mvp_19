    ...
    try {
      ...
      if (json.type === "stripe" && json.url) {
        ...
        if (qrWin) {
          ...
          qrWin.document.write(`<html><body style="text-align:center;font-family:sans-serif;padding:24px;background:#f8fafc"><h3 style="color:#1e293b;margin-bottom:8px">微信扫码支付</h3><p style="color:#64748b;margin-bottom:16px">金额：¥${price}</p><img src="${qrUrl}" style="border-radius:12px;border:1px solid #e2e8f0" /><p style="color:#94a3b8;font-size:12px;margin-top:16px" id="status">请使用微信扫码支付</p><p style="color:#94a3b8;font-size:10px;margin-top:8px" id="debug">调试信息：订单号 ${outTradeNo}</p><script>let checkInterval = setInterval(async () => {try {document.getElementById('debug').textContent = '正在查询支付状态...';const response = await fetch('/api/payment/wechat/check-status', {method: 'POST',headers: {'Content-Type': 'application/json'},body: JSON.stringify({outTradeNo: '${outTradeNo}'})});document.getElementById('debug').textContent = '查询响应：' + response.status;const data = await response.json();document.getElementById('debug').textContent = '查询结果：' + JSON.stringify(data);if (data.ok && data.status === 'paid') {document.getElementById('status').textContent = '支付成功，正在跳转...';clearInterval(checkInterval);setTimeout(() => {window.close();window.opener.location.href = '/market/membership/success?payment=wechat';}, 1500);} else if (data.ok && data.status === 'CLOSED') {document.getElementById('status').textContent = '支付已关闭';clearInterval(checkInterval);} else if (data.ok && data.status === 'PAYERROR') {document.getElementById('status').textContent = '支付失败';clearInterval(checkInterval);} else if (data.ok && data.status === 'unpaid') {document.getElementById('status').textContent = '请使用微信扫码支付';} else if (data.ok) {document.getElementById('status').textContent = '支付状态：' + data.status + (data.originalStatus ? ' (' + data.originalStatus + ')' : '');} else {document.getElementById('status').textContent = '查询失败：' + (data.message || '未知错误');}}, 3000); setTimeout(() => {clearInterval(checkInterval);document.getElementById('status').textContent = '支付超时，请重新尝试';document.getElementById('debug').textContent = '超时：未收到支付结果';}, 30000);</script></body></html>`)
        }
