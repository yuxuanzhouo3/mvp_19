...
export default function VCEmailTemplatesPage() {
  ...
  const loadTemplates = async () => {
    if (!user) return
    setLoading(true)
    try { setTemplates(await loadVCEmailTemplates(user.id)) }
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      if (editId) { await updateVCEmailTemplate(user.id, editId, form); setEditId(null) }
      else await createVCEmailTemplate(user.id, form)
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteVCEmailTemplate(user.id, id); await loadTemplates() }
    catch { setError('删除失败') }
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">VC 邮件模板管理</h1>
