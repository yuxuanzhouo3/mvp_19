...
export default function EnterpriseEmailTemplatesPage() {
  ...
  const loadTemplates = async () => {
    if (!user) return
    setLoading(true)
    try { setTemplates(await loadEnterpriseEmailTemplates(user.id)) }
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      if (editId) { await updateEnterpriseEmailTemplate(user.id, editId, form); setEditId(null) }
      else await createEnterpriseEmailTemplate(user.id, form)
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteEnterpriseEmailTemplate(user.id, id); await loadTemplates() }
    catch { setError('删除失败') }
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">企业邮件模板管理</h1>
