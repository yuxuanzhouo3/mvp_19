...
type AnalyticsTabKey = "overview" | "trends" | "retention" | "habits" | "tools" | "firstUse" | "segments"
...
function shortDate(v: string) {
  const [,m,d] = (v||"").split("-"); return m && d ? `${m}-${d}` : v
}
function fmtDt(v: string) {
  const d = new Date(v); return Number.isFinite(d.getTime()) ? d.toLocaleString() : "-"
}
...
export function AnalyticsDashboardClient() {
  ...
  const load = useCallback(async (d: number) => {
    ...
    try {
      const res = await fetch(`/api/market1/analytics?days=${d}`, { cache: "no-store" })
      if (res.status === 401) { router.replace("/market1/login"); return }
      const json = await res.json().catch(() => ({}))
      if (!res.ok || !json?.success || !json?.analytics) throw new Error(json?.error || "加载失败")
  ...
  const logout = async () => {
    await fetch("/api/market1/auth/logout", { method: "POST" }).catch(() => null)
    router.replace("/market1/login")
  }
    ...
    <div className="min-h-screen bg-muted/20">
      ...
      <div className="h-14 border-b bg-background px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" onClick={() => void load(days)} disabled={loading} className="gap-1.5">
            <RefreshCcw className="h-4 w-4" />
            {loading ? "刷新中..." : "刷新"}
          </Button>
          <Button asChild variant="outline"><Link href="/market1">返回系统导航</Link></Button>
        ...
        <aside className="w-56 border-r bg-background p-3 space-y-1">
          {TABS.map(item => (
            <button key={item.key} onClick={() => setTab(item.key)}
              className={`w-full text-left px-3 py-2 rounded-lg text-sm transition ${tab === item.key ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}>
              {item.label}
            </button>
          ))}
        </aside>
            ...
            <CardHeader className="space-y-3 pb-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {RANGE_OPTIONS.map(opt => (
                  <Button key={opt} size="sm" variant={days===opt?"default":"outline"} onClick={() => setDays(opt)} disabled={loading}>
                    近 {opt} 天
                  </Button>
                ))}
              </div>
              <CardDescription>数据生成时间: {data ? fmtDt(data.generatedAt) : "-"}</CardDescription>
            </CardHeader>
          </Card>
          ...
          {data && (
            <>
              {tab === "overview" && (
                <Card>
                  <CardHeader className="pb-3">
                    <CardTitle className="flex items-center gap-2 text-base"><UsersRound className="h-4 w-4" />核心指标</CardTitle>
                  </CardHeader>
              ...
              {tab === "trends" && (
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-base"><Activity className="h-4 w-4" />活跃与新增趋势</CardTitle>
                  ...
                  <CardContent className="h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={data.trends}>
                        ...
                        <YAxis /><Tooltip labelFormatter={v => `日期 ${v}`} /><Legend />
                        ...
                        <Line type="monotone" dataKey="firstUseUsers" name="首次使用" stroke="#ea580c" strokeWidth={2} dot={false} />
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="overflow-x-auto">
                      <Table>
                        <TableHeader>
                          <TableRow>
                            <TableHead>Cohort 日期</TableHead><TableHead>新增用户</TableHead>
                            <TableHead>D1</TableHead><TableHead>D3</TableHead><TableHead>D7</TableHead><TableHead>D14</TableHead><TableHead>D30</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {data.retention.cohorts.length === 0 ? (
                            <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground">暂无 Cohort 数据</TableCell></TableRow>
                          ) : data.retention.cohorts.slice(0,20).map((row: any) => (
                            <TableRow key={row.cohortDate}>
                              <TableCell>{row.cohortDate}</TableCell><TableCell>{row.newUsers}</TableCell>
                    ...
                    <CardContent className="h-72">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data.habits.byWeekday}>
                          <CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="label" /><YAxis /><Tooltip /><Legend />
                    ...
                    <CardContent className="h-72">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={data.habits.byHour}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="label" tickFormatter={v => v.endsWith(":00") ? v.slice(0,2) : v} minTickGap={18} />
                ...
                <Card>
                  ...
                  <CardContent>
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead>工具</TableHead><TableHead>工具ID</TableHead>
                          <TableHead>事件量</TableHead><TableHead>活跃用户</TableHead><TableHead>事件占比</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {data.habits.topTools.length === 0 ? (
                          <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">暂无工具使用数据</TableCell></TableRow>
                        ) : data.habits.topTools.map((row: any) => (
                          <TableRow key={row.toolId}>
                            <TableCell>{row.toolName}</TableCell>
                            <TableCell className="font-mono text-xs">{row.toolId}</TableCell>
                            <TableCell>{row.events}</TableCell><TableCell>{row.activeUsers}</TableCell>
              ...
              {tab === "firstUse" && (
                <div className="grid gap-4 xl:grid-cols-2">
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2 text-base"><Clock3 className="h-4 w-4" />首次使用工具分布</CardTitle>
                      <CardDescription>新用户首次真正使用的入口工具</CardDescription>
                    </CardHeader>
                    <CardContent className="h-72">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={data.firstUse.topTools} dataKey="users" nameKey="toolName" cx="50%" cy="50%" outerRadius={95}
                            label={(e: any) => `${e.toolName} ${e.share?.toFixed(1)}%`}>
                            {data.firstUse.topTools.map((_: any, i: number) => <Cell key={i} fill={PIE_COLORS[i%PIE_COLORS.length]} />)}
                          </Pie>
                          <Tooltip formatter={(v: any, _: any, p: any) => [v, p?.payload?.toolName || ""]} />
                        </PieChart>
                      </ResponsiveContainer>
                    </CardContent>
                  </Card>
